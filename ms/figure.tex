...
becomes greater than or equal to the CRLB (see \citet{cramer}; \citet{lecam} for proof). 
...
\beq
\text{SNR} \simeq \frac{0.478}{\sigma \gamma} & \text{   for } \beta = 2.5.
\eeq
...
We examine fitting an exact
 ...
 estimates of flux and centroid by optimizing the likelihood using the correct PSF.
 We expect this method to perform best in determing the centroids
...
proportional to $\sigma^{2}$ to the diagonals of $D$ prior to inversion.
...
\eeq 
Then we apply the same 2d second-order polynomial method (\ref{linearfit},\ref{center}) to the 3$\times$3 patch centered on the brightest
...
\item{{\bf Center-of-light centroiding}} \quad In addition to thetwo fitting methods 
...
centroid estimates that saturate the CRLB; with the only exceptions at SNR being less than 10, 
