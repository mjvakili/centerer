...
the matched filter flux estimator $\tilde{F}=\sum_{i}y_{i}P_{i}$. It can be shown that 
\beq
...
We examine fitting an exact
 ...
 of stars, and deliver RMSE very close to Cramer-Rao bound. In the next section,
 we briefly discuss the centroiding methods used in our Cramer-Rao bound saturating tests.
...
\end{eqnarray}
where $Y$ is the image of the star, and $Y^{(s)}$ is the smooth image.

Then, we fit a simple 2d second-order polynomial 
$P(x,y)=a+bx+cy+dx^2+exy+fy^2$ 
to the 3$\times$3 patch centered on the brightest pixel of the convolved image $Y^{s}$.
