...
fast and slow profile-fitting centroiding methods against each other, when they are applied
to relatively low signal-to-noise ratio stars in a wide range of full width at half maxima.
Furthermore, in order to investigate how information-preserving these techniques are, we compare
the root-mean-squared-error arising from these centroiding methods with the Cramer-Rao
...
We find that in comparison with the naive polynomial centroiding, the modified polynomial
cetroiding leads to significant improvements, and gives rise to centroiding errors comparable
to those of the 3$\times$3 quartic approximation method.
