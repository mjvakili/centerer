...
the Fisher information matrix $\mathcal{F}$. Thus, in order to find the CRLB,
 it is sufficient to compute the Fisher matrix. Consider the following log-likelihood 
function. Let us assume that there are $M$ observables $\{f_{1}, ... , f_{M}\}$, each
...
The optimal estimator of flux---one that saturates Cramer-Rao bound---is 
...
$r/\gamma$, CRLB remains proportional to the ratio between $\gamma$ and SNR. For PSF profiles with 
shorter tails, the prefactor of 0.685 in (\ref{crlbmoffat}) becomes smaller. 
