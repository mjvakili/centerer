...
if __name__ == '__main__':
    ...
    if opts.method.startswith('poly'):
       ...
       for i in range(sample):
          ...
          #xs = centererr.spolynom.find_centroid(data , 2. , sigma[i])
       ...
       np.savetxt("erx55_f%d_00r.txt"%(10*fwhm)  ,   np.array(erx)  ,fmt='%.12f')# ,fmt='%.8f')
       np.savetxt("ery55_f%d_00r.txt"%(10*fwhm)  ,   np.array(ery)  ,fmt='%.12f')# ,fmt='%.8f')
       ...
       for i in range(sample):
          ...
          xs = centererr.fitting.fitting_centroid(data , 1. , sigma[i] , fwhm , 2.5)
          #print xs
          #print xc
          erx.append((xs[0]  - xc[0])**2.)
          ery.append((xs[1]  - xc[1])**2.)
       ...
       np.savetxt("erx3_f%d_00rr.txt"%(10*fwhm)   , np.array(erx)   , fmt='%.12f')
       np.savetxt("ery3_f%d_00rr.txt"%(10*fwhm)  , np.array(ery)   , fmt='%.12f')
       ...
       np.savetxt("erx6_f%d_00r.txt"%(10*fwhm)   , np.array(erx)   , fmt = '%.12f')
       np.savetxt("ery6_f%d_00r.txt"%(10*fwhm)   , np.array(ery)   , fmt = '%.12f')
       ...
       np.savetxt("erx66_f%d_00r.txt"%(10*fwhm)   , np.array(erx)   , fmt = '%.12f')
       np.savetxt("ery66_f%d_00r.txt"%(10*fwhm)   , np.array(ery)   , fmt = '%.12f')
       #np.savetxt("flag3_s10_00.txt" , np.array(flag) , fmt = '%.12f')
       #np.savetxt("fwhm3_s10_00.txt" , np.array(fwhm) , fmt = '%.12f')
       F.close()
       ...
       for i in range(sample):
          ...
          #data += np.random.normal(0, sigma[i] , data.shape)
          data = DATA[i]
          xs = centererr.psfpoly.find_centroid(data , fwhm , sigma[i])
          
          erx.append((xs[1]  - xc[0])**2.)
          ery.append((xs[0]  - xc[1])**2.)
          #fwhm.append(f[i])
          
       np.savetxt("erx8_f%d_00r.txt"%(10*fwhm)   , np.array(erx)   , fmt = '%.12f')
       np.savetxt("ery8_f%d_00r.txt"%(10*fwhm)   , np.array(ery)   , fmt = '%.12f')
       #np.savetxt("flag3_s10_00.txt" , np.array(flag) , fmt = '%.12f')
       #np.savetxt("fwhm3_s10_00.txt" , np.array(fwhm) , fmt = '%.12f')
       F.close()
       ...
       for i in range(sample):
          ...
          xs = centererr.efitting.fitting_centroid(data , sigma[i] , fwhm)
          #print xs
          #print xc
          erx.append((xs[0]  - xc[0])**2.) 
          ery.append((xs[1]  - xc[1])**2.)
       ...
       np.savetxt("erx4_f%d_00r.txt"%(10*fwhm)  , np.array(erx) ,fmt='%.12f')
       np.savetxt("ery4_f%d_00r.txt"%(10*fwhm)  , np.array(ery) ,fmt='%.12f')
       F.close()
